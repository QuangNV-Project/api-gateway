...
readonly APP_NAME="api-gateway"
readonly LEGACY_NAME="api-gateway"
...
require_non_empty HEALTHCHECK_IMAGE
...
readonly HEALTH_PATH="${HEALTH_PATH:-/actuator/health/readiness}"
readonly STARTUP_TIMEOUT_SECONDS="${STARTUP_TIMEOUT_SECONDS:-90}"
readonly SOAK_SECONDS="${SOAK_SECONDS:-120}"
readonly PROBE_INTERVAL_SECONDS="${PROBE_INTERVAL_SECONDS:-10}"
readonly LOG_DIRECTORY="${LOG_DIRECTORY:-/logs/api-gateway}"

[[ "$STARTUP_TIMEOUT_SECONDS" =~ ^[1-9][0-9]*$ ]] || fail "STARTUP_TIMEOUT_SECONDS is invalid"
[[ "$SOAK_SECONDS" =~ ^[1-9][0-9]*$ ]] || fail "SOAK_SECONDS is invalid"
[[ "$PROBE_INTERVAL_SECONDS" =~ ^[1-9][0-9]*$ ]] || fail "PROBE_INTERVAL_SECONDS is invalid"
...
eureka_probe() {
    ...
    [[ -n "$zone" ]] || return 1
    zone="${zone%%,*}"
    endpoint="${zone%/}/apps/API-GATEWAY/${EUREKA_INSTANCE_INSTANCE_ID}"
...
restore_previous() {
    ...
    while (( SECONDS < deadline )); do
        if container_exists "$ACTIVE_NAME" && container_running "$ACTIVE_NAME" && health_probe; then
            log "Rollback readiness passed"
            return 0
        fi
        sleep 5
    done
    log "CRITICAL: previous container did not become ready"
    return 1
}
...
if [[ -z "${SERVICE_PORT:-}" ]]; then
    SERVICE_PORT="$(read_env_var API_GATEWAY_PORT || true)"
fi
require_non_empty SERVICE_PORT
[[ "$SERVICE_PORT" =~ ^[1-9][0-9]{0,4}$ ]] && (( SERVICE_PORT <= 65535 )) || fail "SERVICE_PORT is invalid"
docker network inspect "$DOCKER_NETWORK" >/dev/null
mkdir -p "$LOG_DIRECTORY"
docker pull "$IMAGE_REF" >/dev/null
docker pull "$HEALTHCHECK_IMAGE" >/dev/null
...
PHASE="STARTUP_DELAY"
log "Waiting 30 seconds before readiness checks"
sleep 30

PHASE="READINESS"
wait_for_readiness || fail "Candidate readiness did not stabilize"

PHASE="EUREKA"
wait_for_eureka || fail "Candidate did not register as UP in Eureka"

PHASE="SOAKING"
deadline=$((SECONDS + SOAK_SECONDS))
while (( SECONDS < deadline )); do
    candidate_healthy || fail "Candidate failed readiness or restarted during soak"
    eureka_probe || fail "Candidate disappeared from Eureka during soak"
...
COMMITTED=true
log "Deployment committed: environment=${DEPLOY_ENV} image=${IMAGE_REF} active=$(container_id "$ACTIVE_NAME")"
