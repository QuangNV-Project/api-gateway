                ...
                script {
                    ...
                    withCredentials([
                        ...
                        sh """
                            ssh -o StrictHostKeyChecking=no -i $SSH_KEY -p $REMOTE_PORT $REMOTE_USER@$REMOTE_HOST '
                                ENV_FILE=".env.dev"
                                PORT_VAR="API_GATEWAY_PORT"
                                source ./infra/\${ENV_FILE}
                                eval "PORT=\\\$\${PORT_VAR}"

                                echo "Running on Server DEV -> Port: \$PORT"

                                docker run -d --name api-gateway --env-file ./infra/\$ENV_FILE --network dev-network -p \$PORT:\$PORT -v /logs/api-gateway:/app/logs --restart unless-stopped ${DOCKERHUB_IMAGE}:${IMAGE_TAG}
                            '
                        """
                    ...
                    withCredentials([
                        ...
                        sh """
                            ssh -o StrictHostKeyChecking=no -i $SSH_KEY -p $REMOTE_PORT $REMOTE_USER@$REMOTE_HOST '
                                ...
                                docker run -d --name api-gateway --env-file ./infra/\$ENV_FILE --network prod-network -p \$PORT:\$PORT -v /logs/api-gateway:/app/logs --restart unless-stopped ${DOCKERHUB_IMAGE}:${IMAGE_TAG}
                            '
                        """
