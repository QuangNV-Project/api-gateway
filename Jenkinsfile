pipeline {
    ...
    environment {
        ...
        DOCKERHUB_IMAGE = 'quangnv1911/api-gateway'
        HEALTHCHECK_IMAGE = 'curlimages/curl:8.12.1'
        BUILDX_BUILDER = 'api-gateway-builder'
    }
        ...
        stage('Approve production deployment') {
            ...
            steps {
                script {
                    timeout(time: 30, unit: 'MINUTES') {
                        input(
                            id: "api-gateway-prod-${env.BUILD_NUMBER}",
                            message: """
        ...
        always {
            script {
                sendTelegram(
                    ...
                    'See the Jenkins build for deployment and rollback details.'
                )
            }
        }
        cleanup {
        ...
        withEnv(["DOCKER_NETWORK=${config.network}"]) {
            sh '''#!/usr/bin/env bash
                ...
                ssh -o BatchMode=yes -o StrictHostKeyChecking=yes \
                  ...
                  "env DEPLOY_ENV='$DEPLOY_ENV' IMAGE_REF='$IMAGE_REF' IMAGE_VERSION='$IMAGE_VERSION' DOCKER_NETWORK='$DOCKER_NETWORK' REMOTE_ENV_FILE='$REMOTE_ENV_FILE' HEALTHCHECK_IMAGE='$HEALTHCHECK_IMAGE' JENKINS_BUILD_NUMBER='$BUILD_NUMBER' bash -s" \
                  < infra/deploy.sh
            '''
        }
    }
...
def sendTelegram(status, details) {
    ...
    def message = """${statusIcon} <b>api-gateway deployment</b>
